...
interface HistorySellFilterProps {
  search: string;
  pageSize: string;
...
export function HistorySellFilter({
  search,
  pageSize,
  dateRangeFrom,
  dateRangeTo,
}: HistorySellFilterProps) {
  ...
  const hasActiveFilters = search || dateRangeFrom;
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap">
      ...
      <form onSubmit={handleSearchSubmit} className="flex flex-1 max-w-sm items-center gap-2">
        <div className="relative flex-1">
          ...
          <Input
            name="search"
            placeholder="Cari nama customer, barang..."
            defaultValue={search}
            className="pl-8"
          />
        </div>
        ...
        <PopoverTrigger asChild>
          ...
          >
            ...
            {date?.from ? (
              ...
              <span>Pilih Rentang Tanggal Jual</span>
            )}
          </Button>
        </PopoverTrigger>
...
interface HistorySellPaginationProps {
  page: number;
  totalPages: number;
  total: number;
  pageSize: number;
...
export function HistorySellPagination({
  page,
  totalPages,
  total,
  pageSize,
}: HistorySellPaginationProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
